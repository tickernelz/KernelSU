import{_ as s,o as n,c as a,a as l}from"./app.1c178ab8.js";const y=JSON.parse('{"title":"如何為非 GKI 核心整合 KernelSU","description":"","frontmatter":{},"headers":[{"level":2,"title":"使用 kprobe 整合","slug":"using-kprobes","link":"#using-kprobes","children":[]},{"level":2,"title":"手動修改核心原始碼","slug":"modify-kernel-source-code","link":"#modify-kernel-source-code","children":[]}],"relativePath":"zh_TW/guide/how-to-integrate-for-non-gki.md"}'),p={name:"zh_TW/guide/how-to-integrate-for-non-gki.md"},e=l(`<h1 id="introduction" tabindex="-1">如何為非 GKI 核心整合 KernelSU <a class="header-anchor" href="#introduction" aria-hidden="true">#</a></h1><p>KernelSU 可以被整合到非 GKI 核心中，現在它最低支援到核心 4.14 版本；理論上也可以支援更低的版本。</p><p>由於非 GKI 核心的片段化極其嚴重，因此通常沒有統一的方法來建置它，所以我們也無法為非 GKI 裝置提供 Boot 映像。但您完全可以自行整合 KernelSU 並建置核心以繼續使用。</p><p>首先，您必須有能力從您裝置的核心原始碼建置出一個可以開機並且能夠正常使用的核心，如果核心並非開放原始碼，這通常難以做到。</p><p>如果您已經做好了上述準備，那有兩個方法來將 KernelSU 整合至您的核心之中。</p><ol><li>藉助 <code>kprobe</code> 自動整合</li><li>手動修改核心原始碼</li></ol><h2 id="using-kprobes" tabindex="-1">使用 kprobe 整合 <a class="header-anchor" href="#using-kprobes" aria-hidden="true">#</a></h2><p>KernelSU 使用 kprobe 機制來處理核心的相關 hook，如果 <em>kprobe</em> 可以在您建置的核心中正常運作，那麼建議使用這個方法進行整合。</p><p>首先，把 KernelSU 新增至您的核心來源樹狀結構，再核心的根目錄執行以下命令：</p><ul><li>最新 tag (稳定版本)</li></ul><div class="language-sh"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#FFCB6B;">curl</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-LSs</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">&quot;</span><span style="color:#C3E88D;">https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh</span><span style="color:#89DDFF;">&quot;</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">|</span><span style="color:#A6ACCD;"> </span><span style="color:#FFCB6B;">bash</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-</span></span>
<span class="line"></span></code></pre></div><ul><li>main 分支(開發版本)</li></ul><div class="language-sh"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#FFCB6B;">curl</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-LSs</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">&quot;</span><span style="color:#C3E88D;">https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh</span><span style="color:#89DDFF;">&quot;</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">|</span><span style="color:#A6ACCD;"> </span><span style="color:#FFCB6B;">bash</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-s</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">main</span></span>
<span class="line"></span></code></pre></div><ul><li>選取 tag (例如 v0.5.2)</li></ul><div class="language-sh"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#FFCB6B;">curl</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-LSs</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">&quot;</span><span style="color:#C3E88D;">https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh</span><span style="color:#89DDFF;">&quot;</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">|</span><span style="color:#A6ACCD;"> </span><span style="color:#FFCB6B;">bash</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-s</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">v0.</span><span style="color:#F78C6C;">5.2</span></span>
<span class="line"></span></code></pre></div><p>然後，您需要檢查您的核心是否啟用 <em>kprobe</em> 相關組態，如果未啟用，則需要新增以下組態：</p><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#A6ACCD;">CONFIG_KPROBES=y</span></span>
...
<span class="line"><span style="color:#A6ACCD;"></span></span></code></pre></div><p>最後，重新建置您的核心即可。</p><p>如果您發現 KPROBES 仍未生效，很有可能是因為它的相依性 <code>CONFIG_MODULES</code> 並未被啟用 (如果還是未生效請輸入 <code>make menuconfig</code> 搜尋 KPROBES 的其他相依性並啟用)</p><p>如果您在整合 KernelSU 之後手機無法啟動，那麼很可能您的核心中 <strong>kprobe 無法正常運作</strong>，您需要修正這個錯誤，或者使用第二種方法。</p><div class="tip custom-block"><p class="custom-block-title">如何檢查 kprobe 是否損毀？</p><p>將 <code>KernelSU/kernel/ksu.c</code> 中的 <code>ksu_enable_sucompat()</code> 和 <code>ksu_enable_ksud()</code> 取消註解，如果正常開機，即 kprobe 已損毀；或者您可以手動嘗試使用 kprobe 功能，如果不正常，手機會直接重新啟動。</p></div><h2 id="modify-kernel-source-code" tabindex="-1">手動修改核心原始碼 <a class="header-anchor" href="#modify-kernel-source-code" aria-hidden="true">#</a></h2><p>如果 kprobe 無法正常運作 (可能是上游的錯誤或核心版本過低)，那您可以嘗試這種方法：</p><p>首先，將 KernelSU 新增至您的原始碼樹狀結構，再核心的根目錄執行以下命令：</p><div class="language-sh"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#FFCB6B;">curl</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-LSs</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">&quot;</span><span style="color:#C3E88D;">https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh</span><span style="color:#89DDFF;">&quot;</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">|</span><span style="color:#A6ACCD;"> </span><span style="color:#FFCB6B;">bash</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-</span></span>
<span class="line"></span></code></pre></div><p>然後，手動修改核心原始碼，您可以參閱下方的 patch：</p><div class="language-diff"><button title="Copy Code" class="copy"></button><span class="lang">diff</span><pre class="shiki material-palenight has-diff"><code><span class="line"><span style="color:#A6ACCD;">diff --git a/fs/exec.c b/fs/exec.c</span></span>
...
<span class="line"></span></code></pre></div><p>主要修改四個項目：</p><ol><li>do_faccessat，通常位於 <code>fs/open.c</code></li><li>do_execveat_common，通常位於 <code>fs/exec.c</code></li><li>vfs_read，通常位於 <code>fs/read_write.c</code></li><li>vfs_statx，通常位於 <code>fs/stat.c</code></li></ol><p>如果您的核心沒有 <code>vfs_statx</code>，使用 <code>vfs_fstatat</code> 將其取代：</p><div class="language-diff"><button title="Copy Code" class="copy"></button><span class="lang">diff</span><pre class="shiki material-palenight has-diff"><code><span class="line"><span style="color:#A6ACCD;">diff --git a/fs/stat.c b/fs/stat.c</span></span>
...
<span class="line"></span></code></pre></div><p>對於早於 4.17 的核心，如果沒有 <code>do_faccessat</code>，可以直接找到 <code>faccessat</code> 系統呼叫的定義並進行修改：</p><div class="language-diff"><button title="Copy Code" class="copy"></button><span class="lang">diff</span><pre class="shiki material-palenight has-diff"><code><span class="line"><span style="color:#A6ACCD;">diff --git a/fs/open.c b/fs/open.c</span></span>
...
<span class="line"><span style="color:#A6ACCD;"> 		return -EINVAL;</span></span>
<span class="line"></span></code></pre></div><p>若要啟用 KernelSU 內建的安全模式，您還需要修改 <code>drivers/input/input.c</code> 中的 <code>input_handle_event</code> 方法：</p><div class="tip custom-block"><p class="custom-block-title">TIP</p><p>強烈建議啟用此功能，如果遇到開機迴圈，這將會非常有用！</p></div><div class="language-diff"><button title="Copy Code" class="copy"></button><span class="lang">diff</span><pre class="shiki material-palenight has-diff"><code><span class="line"><span style="color:#A6ACCD;">diff --git a/drivers/input/input.c b/drivers/input/input.c</span></span>
...
<span class="line"><span style="color:#A6ACCD;">                add_input_randomness(type, code, value);</span></span>
<span class="line"></span></code></pre></div><p>最後，再次建置您的核心，KernelSU 將會如期運作。</p>`,37),o=[e];function t(c,r,i,C,D,A){return n(),a("div",null,o)}const f=s(p,[["render",t]]);export{y as __pageData,f as default};
